...
namespace Celic
{
    ...
    abstract class BaseTextManager
    {
        ...
        protected void AddParagraph(string text, Formating form = null, Align align = Align.wdAlignParagraphJustify, SpaceParagraph spaceAfter = SpaceParagraph.standart)
        {
            _range = _app.ActiveDocument.Paragraphs.Add().Range;
            _range.Text = text;
            _range.ParagraphFormat.Alignment = align;
            _app.ActiveDocument.Paragraphs.Last.SpaceAfter = (float)spaceAfter;
            if(form != null)
            	form.Invoke();
        ...
        {
            _range = _app.ActiveDocument.Paragraphs.Add().Range;
            _range.Text = "HT = d ∙ mпр =  " + PlastManager.CalcD(plasts[i]) + " ∙ " 
            	+ PlastManager.MPr(plasts[i]) + " =  " + (ht[i] = PlastManager.Ht(plasts[i])) + " м;";
            FormatVariable2("HT");
        ...
        protected bool WriteWordDocument()
        {
            try
            {
        ...
        /// <param name="reporter"> Модель, отслеживающая запись процесса расчета в файл </param>
        protected abstract void WriteCalculation();
        ...
        {
            System.Windows.MessageBox.Show(WriteWordDocument() ? "Файл успешно записан!" : "Файл не удалось записать!");
        }
